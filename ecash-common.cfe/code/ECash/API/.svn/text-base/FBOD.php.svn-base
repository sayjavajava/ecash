...
class ECash_API_FBOD extends ECash_API
{
	...
	protected function getModelClass($index)
	{
		switch($index)
		{
			case self::INDEX_APPLICATION:
				include_once $this->DIR_MODELS . "FBOD{$index}.php";
				return "ECash_Models_FBOD{$index}";
				break;
	...
	protected function getValidatorClass($index)
	{
		...
		{
			case self::INDEX_APPLICATION:
				include_once $this->DIR_VALIDATION . "FBOD{$index}.php";
				return "ECash_Validation_FBOD{$index}";
				break;
