...
require_once( dirname(__FILE__) . "/../../ecash_cfe/sql/lib/scheduling.func.php");
...
//		return $total;
	...
	public function getlinks($data)
	{
		...
		switch (strtolower($data->company_name_short))
		{
			case 'msp':
			case '':
				$data->cs_login_link = "{$esig_url}/es/auto_login?link=". urlencode(base64_encode($app_id))."&key=" . md5($app_id . "24hrloanslogin")."&exp=". urlencode(base64_encode(time())) . "&ecvt&force_new_session" ;
				$data->esig_url = "{$esig_url}/es/esig_confirm_start?application_id=". urlencode(base64_encode($app_id))."&login=" . md5($app_id . "24hrloans") . "&ecvt&force_new_session" ;
				$data->spam_link = "{$esig_url}/es/spam_login?link=". urlencode(base64_encode($app_id))."&key=" . md5($app_id . "24hrloansspam");
				$data->react_link = "{$esig_url}/es/react_loan?link=". urlencode(base64_encode($app_id))."&key=" . md5($app_id . "24hrloansreact")."&exp=". urlencode(base64_encode(time())) . "&ecvt&force_new_session" ;
			break;
			case 'mls':
			case 'multiloansource.net':
			default:
				$data->cs_login_link = "{$esig_url}/auto_login?link=". urlencode(base64_encode($app_id))."&key=" . md5($app_id . "24hrloanslogin")."&exp=". urlencode(base64_encode(time())) . "&ecvt&force_new_session" ;
				$data->esig_url = "{$esig_url}/esig_confirm_start?application_id=". urlencode(base64_encode($app_id))."&login=" . md5($app_id . "24hrloans") . "&ecvt&force_new_session" ;
				$data->spam_link = "{$esig_url}/spam_login?link=". urlencode(base64_encode($app_id))."&key=" . md5($app_id . "24hrloansspam");
				$data->react_link = "{$esig_url}/react_loan?link=". urlencode(base64_encode($app_id))."&key=" . md5($app_id . "24hrloansreact")."&exp=". urlencode(base64_encode(time())) . "&ecvt&force_new_session" ;
			break;	
		}
	...
	protected function checkActive($data)
	{
		...
		//asm 107
		$result = $this->legacy_schedule->fetchLastPayment($data->application_id, $this->app->company_id);
	 ...
	 // Pull out the transactional info needed
			...
			!($this->Is_In_Prefund_Status($this->app)))
		{
			...
			//$this->Fetch_Card_Payments($data); //asm 107 // Do not overwrite last_payment_date,last_payment_amount, card_auth_code
		}
	}
	/**
	...
	{
		/*
		// asm 107, do not allow to overwrite last_payment_date and last_payment_amount
